...
const createEventHeaderTemplate = (item) => {
  return `<header class="event__header">
    ...
    <div class="event__field-group  event__field-group--time">
      ...
      <input class="event__input  event__input--time" id="event-start-time-1" type="text" name="event-start-time" value="${item.duration.start}">
      &mdash;
      <label class="visually-hidden" for="event-end-time-1">To</label>
      <input class="event__input  event__input--time" id="event-end-time-1" type="text" name="event-end-time" value="${item.duration.finish}">
    </div>
...
const createEventOfferTemplate = (arr) => {
  let list = ``;
...
const createEventDestinationTemplate = (item) => {
  ...
  for (let i = 0; i < item.photoGallery.length; i++) {
    imgGallery += `<img class="event__photo" src="${item.photoGallery[i]}" alt="Event photo">`;
  }
